...
REPOSITORIES_URL = "https://api.github.com/graphql/repositories"
REMAINING_RATELIMIT_HEADER = "X-RateLimit-Remaining"


def clean_graphql_data(res):
    cleaned_data = []
    for repo in res:
        new_repo = copy(repo['node'])
        new_repo['cursor'] = repo['cursor']
        new_repo['languages'] = {lang['node']['name']: lang['size'] for lang in new_repo['languages']['edges']}
        new_repo['stargazers'] = new_repo['stargazers']['totalCount']
        if new_repo['primaryLanguage']:
            new_repo['primaryLanguage'] = new_repo['primaryLanguage']['name']
        cleaned_data.append(new_repo)
    return cleaned_data
...
def crawl(
    ...
    github_token = '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' # TODO(jk): Fix
    if github_token is not None and github_token != "":
        headers["Authorization"] = f"token {github_token}"

    curr_rate_limit = min_rate_limit + 10
    while cursor is not None and count < limit and curr_rate_limit > min_rate_limit:
        time.sleep(interval)
        req = requests.post(GRAPHQL_URL, json={'query': get_new_repos(cursor=cursor).replace("first: 100", "first: 5")}, headers=headers)
        print("Got request back")
        response_body = req.json()
        if not response_body:
            break

        data = clean_graphql_data(response_body["data"]["search"]["edges"])
        print("Cleaned data")
        result["data"].extend(data)  # type: ignore
        cursor = data[-1].get("cursor")
        count += len(data)

        curr_rate_limit_raw = req.headers.get(REMAINING_RATELIMIT_HEADER)
        try:
            curr_rate_limit = -1
    ...
    result["repositoryCount"] = response_body["data"]["search"]["repositoryCount"]
    result["command"] = "graphql"
...
@click.option(
    ...
    type=str,
    default="",
    help="last cursor seen from last crawl; current crawl will start from its successor",
)
@click.option(
    "--max",
    "-m",
...
def crawl_handler(
    ...
    while count < max:
        result = crawl(
            cursor,
            count,
            min(count + batch_size, max),
            interval,
            min_rate_limit,
        )
        # TODO(jk): Write to RDS

        outfile = os.path.join(crawldir, f"{cursor}.json")
